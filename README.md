...
If the Windows `py` launcher is not available, use `python mini.py` instead.
...
2. Make sure the directory contains a command wrapper named `mini`.

On Windows, this repository includes `mini.cmd` beside `mini.py`:
...
After adding the project directory to `PATH`, run `mini -h` from any terminal.

On macOS or Linux, create a `mini` shell wrapper or symlink that runs `mini.py`.

3. Add the project directory to your `PATH` environment variable. E.g. `C:\Program Files\mini-lang` or `/usr/local/bin/mini-lang`.

4. Open a new terminal and run the `mini` interpreter from any directory.
